...
static const char OV5642_InitRegTable[][3] = {
    { 0x31, 0x03, 0x93 },
    { 0x30, 0x08, 0x82 },
    { 0x30, 0x17, 0x7f },
    { 0x30, 0x18, 0xfc },
...
#define SIZE_OV5642_InitRegTable    (sizeof(OV5642_InitRegTable)/sizeof(uint8_t))

static const char OV5642_sw_reset_cmd[3] = { 0x30, 0x00, 0xf8 };
#define SIZE_OV5642_sw_reset_cmd    (sizeof(OV5642_sw_reset_cmd)/sizeof(uint8_t))

#if 0
static int OV5642_reg_write(uint8_t reg1, uint8_t reg2, uint8_t v) {
    return sccb2_reg_write(OV5642_ADDR, reg1, reg2, v);
}
#endif

static int OV5642_reg_write_n(const uint8_t *tbl, size_t size) {
    return sccb2_reg_write_n(OV5642_ADDR, tbl, size);
}
...
public:
    ...
    virtual bool Initialise() {
        ...
        ret = OV5642_reg_write_n((const uint8_t *)OV5642_sw_reset_cmd, SIZE_OV5642_sw_reset_cmd);
        if (ret != 0) {
            return false;
        }
        ThisThread::sleep_for(1ms);

        ret = OV5642_reg_write_n((const uint8_t *)OV5642_InitRegTable, SIZE_OV5642_InitRegTable);
        if (ret != 0) {
            return false;
        }
        return true;
