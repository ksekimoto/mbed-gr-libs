...
JPEG_Converter::decode(void* pJpegBuff, bitmap_buff_info_t* psOutputBuff, decode_options_t* pOptions) {
    ...
    if (driver_ac_count > 0) {
        ...
        if (pOptions->p_DecodeCallBackFunc == NULL) {
            jpeg_converter_semaphore.try_acquire_for(rtos::Kernel::wait_for_u32_forever); // WAIT
        } else {
            if (!jpeg_converter_semaphore.try_acquire_for(rtos::Kernel::Clock::duration_u32(0))) {
